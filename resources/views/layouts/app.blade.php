...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <title>@if(isset(auth()->user()->currentTeam->name)){{ auth()->user()->currentTeam->name }}@else{{ config('app.name', 'Climate Clock') }}@endif</title>
            ...
            @if(Request::get('onboard'))
                <x-modal max-width="lg" x-data="{ show: true }" :show="true" class="p-10">
                    ...
                    <div class="p-5">
                        ...
                        <p class="py-4 text-lg text-gray-500">This is your community dashboard. On your dashboard you can post a quick message in your community feed, set your status to available, and view your climate community status.</p>
                        <p class="py-2 pb-4 text-lg text-gray-500" target="_blank">Next, you may want to invite a few more members to your climate community by telling them to signup at <a href="{{ url('register') }}" class="text-green-400 underline">{{ url('register') }}</a></p>
